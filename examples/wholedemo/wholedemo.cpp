...
void* freenect_threadfunc(void* arg) {
  uint32_t lastTimestampDepth = 0;
  uint32_t lastTimestampRGB = 0;
  ...
  while(true)  {
      ...
      KinectIO::instance().kinect().getDepth(lastTimestampDepth, depth);
      KinectIO::instance().kinect().getRgb(lastTimestampRGB, rgb);
      ...
      //myview.setBuffer(rgbMat.data,spec,0);
